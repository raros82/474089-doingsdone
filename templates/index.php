...
<div class="tasks-controls">
    ...
    <nav class="tasks-switch">
        <a href="/?filter=all_tasks"
           class="tasks-switch__item <?php if ($filter == 'all_tasks') : ?>tasks-switch__item--active<?php endif; ?>">Все
            задачи</a>
        <a href="/?filter=agenda"
           class="tasks-switch__item <?php if ($filter == 'agenda') : ?>tasks-switch__item--active<?php endif; ?>">Повестка
            дня</a>
                    ...
                    <td class="task__select">
                        <label class="checkbox task__checkbox">
                            ...
                            <input class="checkbox__input visually-hidden task__checkbox"
                                   type="checkbox" <?php if ($tasks_value['task_status'] == '1'): ?> checked <?php endif; ?>
                                   value="<?= $tasks_value['task_id']; ?>">
                            <span class="checkbox__text"><?= esc($tasks_value['task_name']); ?></span>
                        </label>
                    </td>
                    <td class="task__date"><?php deadline($tasks_value['deadline']); ?></td>
                    <td class="task__controls"><a href="<?php echo $tasks_value['file_atach']; ?> "
                                                  target="_blank"> <?php if (isset($tasks_value['file_atach'])) {
                                echo 'файл';
                            } ?> </a></td>
                </tr>
            <?php endif; ?>
